...
1단계
*/
HRESULT Input::InitInput(HWND hWnd)
{
	m_hWnd = hWnd;
	...
	//마우스 협조 레벨을 얻어온다.
	if (m_pMouse->SetCooperativeLevel(m_hWnd, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND))
	{
...
HRESULT Input::ReadData()
{
	...
	if (FAILED(m_pMouse->GetDeviceState(sizeof(m_MouseState2), &m_MouseState2)))
	{
...
*/
void Input::OnActivate(WPARAM wParam)
{
